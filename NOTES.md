...
Internal working log — sub-step detail, design reasoning, and the shipped
changelog behind each commit. The README stays outward-facing; this is where
the play-by-play lives. Companion to [`docs/v2-backlog.md`](docs/v2-backlog.md)
(the Step 7/8 reasoning trail) and
[`docs/step9-mobile-spec.md`](docs/step9-mobile-spec.md) (the mobile build
spec).
...
- **7.1 Yeti max turn rate** — a hard cap on how fast the yeti's heading
  ...
  unfair without it.
- **7.2 Mirror / look-behind** — the camera glances behind you, the view
...
- **7.3 Footprints** — the player leaves tracks in snow. An investigating
...
- **7.4 Inventory** — 4 generic carried slots. Pick an item into whatever
...
- **7.5 Dropped-item hint pip** — a fuzzy HUD direction pip, no distance,
  back toward anything you've dropped. Mainly for the blanket (7.6); applies to
  any dropped slot item.
- **7.6 Placeable blanket** — the blanket becomes placeable: set it down,
  warmth drains slower while you stand on it, 7.5 points you back to it.
- **7.7 Snack / water-bottle merge** — snack is the base consumable (locks
...
- **7.10 Pause** — **Esc** opens a pause menu and releases the mouse in one
  ...
  isn't reliable). Click **Resume** to re-lock.
...
Within a group the exact order is playtest-driven, like 6.10–6.14. All of Step 7
...
a phone for playtesting. Desktop keyboard/mouse stays the default and is left
untouched — the touch layer is swapped in on a coarse-pointer device. Same
discipline as Steps 6–8: one session and one commit per item, a playtest
between each, push after each. Order within the step is playtest-driven, like
6.10–6.14.
...
- **9.3 On-screen action buttons** — glance behind, snack/blanket, decoy as
  thumb-reachable tap targets, clear of the joystick and look zones.
